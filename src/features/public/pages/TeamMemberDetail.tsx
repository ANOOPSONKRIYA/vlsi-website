...
export function TeamMemberDetail() {
  ...
  const getSocialIcon = (platform: string) => {
    switch (platform) {
      ...
      case 'google-scholar': return BookOpen;
        ...
        >
          ...
          <div className={`relative px-4 sm:px-6 lg:px-8 pb-6 sm:pb-8 ${member.coverImage ? '-mt-12 sm:-mt-16 lg:-mt-20' : 'pt-6 sm:pt-8'}`}>
            <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
              ...
              {member.resume?.url && (
                <a
                  href={normalizeExternalUrl(member.resume.url)}
                  target="_blank"
          ...
          <div className="lg:col-span-2 space-y-4 sm:space-y-6 lg:space-y-8">
            ...
            {member.about?.trim() && (
              ...
              >
                ...
                <p className="text-white/50 text-xs sm:text-sm lg:text-base leading-relaxed whitespace-pre-line">
                  {member.about}
                </p>
              </motion.section>
            )}
                  ...
                  {member.achievements.map((achievement) => (
                    <div key={achievement.id} className="flex items-start gap-2 sm:gap-3">
                      ...
                      <div>
                        ...
                        {achievement.link?.trim() && (
                          <a
                            href={normalizeExternalUrl(achievement.link)}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1 text-white/50 hover:text-white text-[10px] sm:text-xs mt-1"
                          >
                            Website
